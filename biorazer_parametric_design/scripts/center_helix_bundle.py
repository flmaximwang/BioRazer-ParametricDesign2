...
def write_pml(pml_path, axis_length):
    """生成 PyMOL 脚本: 只绘制 x/y/z 三轴 (不载入结构)。

    注意: pml 由 PyMOL 自带的解析器处理, 顶层 Python 会被逐行执行,
    因此所有 Python 代码必须放进 ``python ... python end`` 块;
    顶层注释保持纯 ASCII 且不含 ';'。
    """
    pml = f'''# PyMOL script: x/y/z axes only (centered bundle at origin)
# Generated by center_helix_bundle.py
# Load the structure yourself, then run:  pymol -q {pml_path.name}
# Axes use biorazer_pymol.mark.arrow.arrow_pass when importable
# (installed in /opt/envs/pymol), otherwise a built-in CGO fallback
...
def main(argv=None):
    ...
    print(f"PyMOL 脚本已写出: {pml_path} (只画 x/y/z 轴)")
    parent = pml_path.parent
    print(f"运行: {'cd ' + str(parent) + ' && ' if str(parent) != '.' else ''}pymol -q {pml_path.name}  (需先自行载入结构)")
